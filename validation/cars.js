...
module.exports = function validateAddCarInput (data) {
  ...
  if (!Validator.isLength(data.year, { min: 4, max: 4 })) {
    errors.year = 'Year must be between 4 and 4 numbers'
  }
  if (Validator.isEmpty(data.year)) {
    errors.year = 'Year field is required'
  }
  if (!Validator.isLength(data.mpg, { min: 1, max: 2 })) {
    errors.mpg = 'Year must be between 1 and 2 numbers'
  }
  if (Validator.isEmpty(data.mpg)) {
    errors.mpg = 'MPG field is required'
  }
  if (!Validator.isLength(data.realMpg, { min: 1, max: 2 })) {
    errors.realMpg = 'Real MPG must be between 1 and 2 numbers'
  }
  if (Validator.isEmpty(data.realMpg)) {
    errors.realMpg = 'Real MPG field is required'
  }
  if (!Validator.isLength(data.hp, { min: 1, max: 3 })) {
    errors.hp = 'HP must be between 2 and 3 numbers'
  }
  if (Validator.isEmpty(data.hp)) {
    errors.hp = 'HP field is required'
  }
  if (!Validator.isLength(data.engine, { min: 4, max: 4 })) {
    errors.engine = 'Engine must be between 4 and 4 numbers'
  }
